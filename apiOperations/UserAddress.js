...
async function getAllAddress(UserID) {
  try {
    ...
    var result = await pool
      ...
      .query(
        "SELECT ua.*, c.COUNTRY_NAME, s.STATE_NAME, ci.CITY_NAME, a.AREA_NAME, (select case when count(*) > 0 then 1 else 0 end as cnt from [dbo].[ORDERS] where [ORDERS_ADDRESS_FKID] = ua.USER_ADDRESS_PKID) as ADDRESS_LINKED_STATUS FROM [USER_ADDRESS] ua join [dbo].[COUNTRY] c on c.COUNTRY_PKID = ua.USER_ADDRESS_COUNTRY_FKID join [dbo].[STATE] s on s.STATE_PKID = ua.USER_ADDRESS_STATE_FKID join [dbo].[CITY] ci on ci.CITY_PKID = ua.USER_ADDRESS_CITY_FKID  join [dbo].[AREA] a on a.AREA_PKID = ua.USER_ADDRESS_AREA_FKID where USER_ADDRESS_USER_FKID = @USER_ADDRESS_USER_FKID"
      );
...
async function getUserDefaultAddress(UserID) {
  try {
    ...
    var result = await pool
      ...
      .input("USER_ADDRESS_USER_FKID", UserID)
      .query(
        "SELECT ua.*, c.COUNTRY_NAME, s.STATE_NAME, ci.CITY_NAME, ar.AREA_NAME FROM [USER_ADDRESS] ua join [dbo].[COUNTRY] c on c.COUNTRY_PKID = ua.USER_ADDRESS_COUNTRY_FKID join [dbo].[STATE] s on s.STATE_PKID = ua.USER_ADDRESS_STATE_FKID join [dbo].[CITY] ci on ci.CITY_PKID = ua.USER_ADDRESS_CITY_FKID join [dbo].[AREA] ar on ar.AREA_PKID = ua.USER_ADDRESS_AREA_FKID where USER_ADDRESS_USER_FKID = @USER_ADDRESS_USER_FKID and USER_ADDRESS_DEFAULT = 1"
      );
    return result.recordsets[0];
  } catch (error) {
...
async function addAddress(obj) {
  try {
    ...
    var insertInto = await pool
      ...
      .input("USER_ADDRESS_AREA_FKID", obj.USER_ADDRESS_AREA_FKID)
      ...
      .query(
        "insert into USER_ADDRESS (USER_ADDRESS_NAME, USER_ADDRESS_PHONE, USER_ADDRESS_TYPE, USER_ADDRESS_HOUSE_NO, USER_ADDRESS_STREET, USER_ADDRESS_CITY_FKID, USER_ADDRESS_STATE_FKID, USER_ADDRESS_COUNTRY_FKID, USER_ADDRESS_ZIPCODE, USER_ADDRESS_DEFAULT, USER_ADDRESS_USER_FKID, USER_ADDRESS_DATE, USER_ADDRESS_AREA_FKID)  values(@USER_ADDRESS_NAME, @USER_ADDRESS_PHONE, @USER_ADDRESS_TYPE, @USER_ADDRESS_HOUSE_NO, @USER_ADDRESS_STREET, @USER_ADDRESS_CITY_FKID, @USER_ADDRESS_STATE_FKID, @USER_ADDRESS_COUNTRY_FKID, @USER_ADDRESS_ZIPCODE, @USER_ADDRESS_DEFAULT, @USER_ADDRESS_USER_FKID,GETDATE(),@USER_ADDRESS_AREA_FKID)"
      );
...
async function updateAddress(AddressID, obj) {
  try {
    ...
    var result = await pool
      ...
      .input("USER_ADDRESS_USER_FKID", obj.USER_ADDRESS_USER_FKID)
      .query(
        `UPDATE USER_ADDRESS SET USER_ADDRESS_AREA_FKID = @USER_ADDRESS_AREA_FKID, USER_ADDRESS_NAME = @USER_ADDRESS_NAME, USER_ADDRESS_PHONE= @USER_ADDRESS_PHONE, USER_ADDRESS_TYPE = @USER_ADDRESS_TYPE, USER_ADDRESS_HOUSE_NO=@USER_ADDRESS_HOUSE_NO, USER_ADDRESS_STREET=@USER_ADDRESS_STREET, USER_ADDRESS_CITY_FKID=@USER_ADDRESS_CITY_FKID, USER_ADDRESS_STATE_FKID=@USER_ADDRESS_STATE_FKID, USER_ADDRESS_COUNTRY_FKID=@USER_ADDRESS_COUNTRY_FKID,USER_ADDRESS_ZIPCODE=@USER_ADDRESS_ZIPCODE,USER_ADDRESS_DEFAULT=@USER_ADDRESS_DEFAULT,USER_ADDRESS_USER_FKID=@USER_ADDRESS_USER_FKID where USER_ADDRESS_PKID=@USER_ADDRESS_PKID`
      );
